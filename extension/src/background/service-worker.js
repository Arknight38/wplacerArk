/**
 * Background Service Worker - Handles extension lifecycle and token management
 * Enhanced with dynamic WASM detection and improved pawtect integration
 */
...
const getSettings = async () => {
    ...
    return {
        // Default to 3000 to match the updated extension
...
const pollForTokenRequest = async () => {
    ...
    try {
        const url = await getServerUrl("/api/system/token-needed");
...
const sendCookie = async (callback) => {
    ...
    const url = await getServerUrl("/api/users");
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    if (request.type === "SEND_TOKEN") {
        getServerUrl("/api/system/token").then(url => {
            const payload = {
                t: request.token,
        ...
        }).catch(err => console.error('wplacer: Failed to resolve server URL for /api/system/token', err));
    }
    return false;
});
