...
import re, time, sys, pathlib, requests, os, json, itertools
...
def login_once(email, password):
    ...
    try:
        ...
        print(f"[INFO] Redirected to Google login URL: {google_login_url}")
    except Exception as e:
    ...
    with Camoufox(headless=None, humanize=0.1, screen=Screen(max_width=200, max_height=400), proxy=tor_proxy) as browser:
...
def get_ip_via_tor():
    ...
    return requests.get("https://api.ipify.org", proxies=proxies, timeout=30).text.strip()
...
def main():
    state = load_state()
    print(f"[IP] {get_ip_via_tor()}")
    n = len(state["accounts"])
    start = next((i for i,a in enumerate(state["accounts"]) if a.get("status")=="pending"), n)
    for i in range(start, n):
        process_account(state, i)
    state["cursor"]["next_index"] = n
    save_state(state)
    print("[DONE]")
